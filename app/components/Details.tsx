...
const ScoreBadge = ({score}: { score: number }) => {
    return (
        <div className={
            cn("flex flex-row gap-1 items-center px-2 py-0.5 rounded[96px]",
                score > 69
                    ? "bg-badge-green"
                    : score > 39
                        ? "bg-badge-yellow"
    ...
    return (
        <div className="flex flex-col gap-4 items-center w-full">
            <div className="bg-gray-50 w-full rounded-lg px-5 py-4 grid grid-cols-2 gap-4">
                {tips.map((tip, index) => (
                    <div className="flex flex-row gap-2 items-center" key={index}>
                        <img src={tip.type === "good" ? "/icons/check.svg" : "/icons/warning.svg"} alt="score"
                             className="size-5"/>
            ...
            <div className="flex flex-col gap-4 w-full">
                {tips.map((tip, index) => (
                    <div className={
                        cn("flex flex-col gap-2 rounded-2xl p-4",
                            tip.type === "good"
